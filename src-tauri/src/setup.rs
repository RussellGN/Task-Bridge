...
fn setup_deep_linking(app: &mut App<impl Runtime>) -> Result<(), Box<dyn Error>> {
   ...
   match deep_link.is_registered(APP_URL_SCHEME) {
      Err(e) => {
         ...
         log!("[setup_deep_linking] attempting to register scheme: {APP_URL_SCHEME}");
         match deep_link.register(APP_URL_SCHEME) {
            Ok(_) => {
               log!("[setup_deep_linking] '{APP_URL_SCHEME}' scheme registration was successfull");
               log!(
            ...
            Err(e) => {
               log!("[setup_deep_linking] '{APP_URL_SCHEME}' scheme registration failed. Error: {e}");
      ...
      Ok(is_reg) => {
         log!("is app registered to handle 'task-bridge' urls? {is_reg}");
      }
   }
   ...
   deep_link.on_open_url(move |e| {
      let urls = e.urls();
      log!("[on_open_url] deep link hit. Urls: {urls:#?}");
            ...
            Some(url) => match url.path() {
               "/proceed-to-auth" => {
                  if let Err(e) = proceed_to_auth(url, &app_handle) {
                     log!("[on_open_url] error handling deep-link-hit: {e}");
                  }
               }
               path => log!("[on_open_url] no handler set for deep link path '{path}', url = {url:#?}"),
            },
